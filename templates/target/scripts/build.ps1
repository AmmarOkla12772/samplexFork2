...
$BOARD_DIR = Resolve-Path "$PSScriptRoot/.."
$BUILD_DIR = Join-Path $BOARD_DIR "build"
$NUM_JOBS = 4

Write-Host "=========================================="
Write-Host "Target Board Build Script (PowerShell)"
Write-Host "=========================================="
...
if (!$armGcc -and !$env:ARM_GCC_PATH) {
    Write-Host "[WARNING] arm-none-eabi-gcc not found on PATH." -ForegroundColor Yellow
}
