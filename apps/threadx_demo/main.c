...
/* Compile-time specification checks to ensure board compatibility */
#ifndef BSP_RAM_END
#error                                                                         \
    "The target board's board_config.h must define BSP_RAM_END to establish ThreadX memory boundaries!"
#endif

#ifndef BSP_SYSTEM_CLOCK_HZ
#error                                                                         \
    "The target board's board_config.h must define BSP_SYSTEM_CLOCK_HZ for system clock frequency!"
#endif
...
#define MIN_DYNAMIC_HEAP_REQUIRED 10240
...
static void queue_sender_entry(ULONG parameter) {
  ...
  while (1) {
    ...
    if (tx_block_allocate(&sensor_block_pool, (VOID **)&data_ptr, TX_NO_WAIT) ==
        TX_SUCCESS) {
...
void tx_application_define(void *first_unused_memory) {
  ...
  pool_size = (BSP_RAM_END - 4096) - (ULONG)first_unused_memory;

  /* Fail early if remaining RAM is insufficient for the demo's threads and
   * queues */
  if (pool_size < MIN_DYNAMIC_HEAP_REQUIRED) {
    printf("\r\n==========================================\r\n");
    printf("FATAL ERROR: Insufficient RAM for ThreadX Demo!\r\n");
    printf("Required: %lu bytes, Available: %lu bytes\r\n",
           (ULONG)MIN_DYNAMIC_HEAP_REQUIRED, pool_size);
    printf("==========================================\r\n");
    while (1) {
    }
  }
  ...
  status = tx_byte_allocate(&byte_pool, &block_pool_ptr, block_pool_size,
                            TX_NO_WAIT);
  if (status != TX_SUCCESS) {
  ...
  status =
      tx_block_pool_create(&sensor_block_pool, "Sensor Block Pool",
                           sizeof(SensorData), block_pool_ptr, block_pool_size);
  if (status != TX_SUCCESS) {
  ...
  if (status == TX_SUCCESS) {
    status = tx_thread_create(
        &semaphore_thread, "semaphore thread", semaphore_thread_entry, 0,
        stack_ptr, THREAD_STACK_SIZE, 12, 12, TX_NO_TIME_SLICE, TX_AUTO_START);
    if (status != TX_SUCCESS) {
