...
```text
samplexFORK/ (repository root)
├── libs/                           # Shared RTOS components (ThreadX, NetXDuo)
├── MXChip/                         # [Legacy] Existing standalone board sample
├── targets/                        # [Framework] Supported BSP target boards
│   └── STMicroelectronics/
│       └── NUCLEO_F401RE/          # Board-specific BSP implementation
├── bsp/                            # [Framework] Abstract BSP interface definitions
│   └── include/bsp/                # board.h, led.h, console.h
├── apps/                           # [Framework] Platform-independent applications
│   └── threadx_demo/               # ThreadX demonstration application
...
* `void bsp_console_write(const char *data, int length)`: Transmits a block of data over the console interface.
